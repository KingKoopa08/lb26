  ...
  app.get('/api/admin/analytics', requireAdmin, async (request, response, next) => {
    try {
      ...
      const [summary,trend,sources,pages,campaigns,devices,browsers,countries,conversions] = await Promise.all([
        ...
        pool.query(`SELECT to_char(calendar.day,'YYYY-MM-DD') day,COALESCE(page_views,0)::int page_views,COALESCE(visitors,0)::int visitors FROM generate_series(current_date-($1::int-1),current_date,interval '1 day') AS calendar(day) LEFT JOIN (SELECT date_trunc('day',occurred_at) d,count(*) FILTER (WHERE event_type='page_view') page_views,count(DISTINCT visitor_id) visitors FROM web_analytics_events WHERE ${since} GROUP BY 1) x ON x.d=calendar.day ORDER BY calendar.day`,values),
